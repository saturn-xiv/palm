...
void palm::rabbitmq::Connection::declare_queue(const std::string& name) {
  amqp_queue_declare_ok_t* reply =
      amqp_queue_declare(this->connection, 1, amqp_cstring_bytes(name.c_str()),
...
void palm::rabbitmq::Connection::consume(const std::string& queue,
  ...
  {
    // FIXME ack mode
    amqp_basic_consume(this->connection, 1, amqp_cstring_bytes(queue.c_str()),
  ...
  //   amqp_envelope_t envelope;
  //   const auto it = amqp_consume_message(this->connection, &envelope, NULL,
  //   0);
...
std::shared_ptr<palm::rabbitmq::Connection> palm::rabbitmq::Config::build() {
  ...
  {
    const auto status =
        amqp_socket_open(sck, this->host.c_str(), static_cast<int>(this->port));
    if (status != 0) {
...

  {
    ...
    const auto reply = amqp_login(con, this->virtual_host.c_str(), 0, 131072, 0,
