...
//     auto re = (redisReply*)redisCommand(this->context, "TTL %s",
//     key.c_str()); int ttl = re->integer; freeReplyObject(re);
...
//     reply =
//         (redisReply*)redisCommand(this->context, "SET %s %s EX %d",
//         key.c_str(),
//                                   val.c_str(), ttl.value().count());
//   } else {
//     reply = (redisReply*)redisCommand(this->context, "SET %s %s",
//     key.c_str(),
//                                       val.c_str());
//   }
...
//   const auto key = this->key(key_);
//   auto reply = (redisReply*)redisCommand(this->context, "GET %s",
//   key.c_str()); const std::string val = reply->str; BOOST_LOG_TRIVIAL(debug)
//   << val; freeReplyObject(reply); return val;
// }
