...
macro_rules! to_datetime {
    ($x:expr) => {{ chrono::NaiveDateTime::from_timestamp_opt($x.seconds, $x.nanos as u32).unwrap_or_default() }};
}

#[macro_export]
macro_rules! to_chrono_duration {
    ($x:expr) => {{ chrono::Duration::seconds($x.seconds) }};
}

#[macro_export]
macro_rules! to_std_duration {
    ($x:expr) => {{ std::time::Duration::new($x.seconds as u64, $x.nanos as u32) }};
}
...
macro_rules! try_grpc {
    ($r:expr, $e:expr) => {{ $r.map_err($e) }};
    ($r:expr) => {{ try_grpc!($r, |e| tonic::Status::internal(e.to_string())) }};
}

#[macro_export]
macro_rules! try_web {
    ($r:expr, $e:expr) => {{ $r.map_err($e) }};
    ($r:expr) => {{ try_web!($r, actix_web::error::ErrorInternalServerError) }};
}
...
use std::fs::{File, copy as copy_file, create_dir_all, read_dir, remove_dir_all, remove_file};
use std::io::{Error as IoError, ErrorKind as IoErrorKind, Result as IoResult, prelude::*};
...
pub const FLATBUFFERS: &str = "application/x-flatbuffer";

pub fn timestamp2datetime(ts: i64) -> Option<DateTime<Utc>> {
