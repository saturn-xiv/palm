#include "gourd/env.hpp"
#include "gourd/version.hpp"
...
gourd::Config::Config(const std::filesystem::path& file) {
...
std::string gourd::Jwt::verify(const std::string& token,
                                const std::optional<std::string> audience) {
  spdlog::debug("{}", token);
  auto validator_b =
...
std::unique_ptr<crypto::tink::JwtMac> gourd::Jwt::load() {
...
std::string gourd::HMac::sign(const std::string& plain) {
  auto mac = this->load();
...
void gourd::HMac::verify(const std::string& code, const std::string& plain) {
  auto mac = this->load();
  auto status = mac->VerifyMac(code, plain);
  this->check(status);
}

std::unique_ptr<crypto::tink::Mac> gourd::HMac::load() {
...
std::string gourd::Aes::encrypt(const std::string& plain) {
  auto aes = this->load();
...
std::string gourd::Aes::decrypt(const std::string& code) {
  auto aes = this->load();
...
std::unique_ptr<crypto::tink::Aead> gourd::Aes::load() {
...
std::unique_ptr<crypto::tink::KeysetHandle> gourd::Keyset::load(
...
std::string gourd::auth(const std::string& token) {
  gourd::Jwt jwt(gourd::PROJECT_NAME);
