...
std::string marguerite::Jwt::sign(
  ...
  if (payload) {
    raw_rb = raw_rb.AddJsonObjectClaim(marguerite::Jwt::PAYLOAD_KEY,
                                       payload.value());
  }
...
marguerite::Jwt::verify(const std::string& token, const std::string& issuer,
  ...
  if (payload.HasJsonObjectClaim(marguerite::Jwt::PAYLOAD_KEY)) {
    auto ir = payload.GetJsonObjectClaim(marguerite::Jwt::PAYLOAD_KEY);
    this->check(ir);
    auto iv = std::move(ir.value());
...
std::unique_ptr<crypto::tink::JwtMac> marguerite::Jwt::load() {
...
std::string marguerite::HMac::sign(const std::string& plain) {
  auto mac = this->load();
...
void marguerite::HMac::verify(const std::string& code,
...
std::unique_ptr<crypto::tink::Mac> marguerite::HMac::load() {
...
std::string marguerite::Aes::encrypt(const std::string& plain) {
  auto aes = this->load();
...
std::string marguerite::Aes::decrypt(const std::string& code) {
  auto aes = this->load();
...
std::unique_ptr<crypto::tink::Aead> marguerite::Aes::load() {
...
std::unique_ptr<crypto::tink::KeysetHandle> marguerite::Keyset::load(
