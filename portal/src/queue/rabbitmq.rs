...
pub trait ProtobufMessageSender {
    fn publish<V: ProtobufMessage>(
        &self,
        exchange: &str,
        routing_key: &str,
        task: &V,
...
impl ProtobufMessageSender for Client {
    async fn publish<V: ProtobufMessage>(
        &self,
        exchange: &str,
        routing_key: &str,
        task: &V,
...
pub trait FlexBuffersMessageSender {
...
impl FlexBuffersMessageSender for Client {
...
impl<H: super::ProtobufConsumer> super::Consumer for ProtobufConsumer<H> {
    ...
    async fn consume(&self, id: &str, content_type: &str, payload: &[u8]) -> Result<()> {
        ...
        let it = H::Message::parse(payload)?;
