...
coconut::redis::Config::Config(const toml::table& node) {
...
sw::redis::Redis coconut::redis::Config::open() const {
  auto options = this->connection_options();
  auto con = sw::redis::Redis(options);
  return std::move(con);
}

sw::redis::ConnectionOptions coconut::redis::Config::connection_options()
    const {
...
coconut::redis::Config::connection_pool_options() const {
