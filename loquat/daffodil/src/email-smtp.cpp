...
coconut::smtp::Config::Config(const toml::table& node) {
  ...
  this->_password = node["password"].value<std::string>().value();
  {
    ...
    if (tmp) {
      ...
      this->_from = std::make_shared<coconut::smtp::Address>(*from);
    } else {
...
coconut::smtp::Address::Address(const toml::table& node) {
  this->username = node["username"].value<std::string>().value();
  this->email = node["email"].value<std::string>().value();
}

void coconut::smtp::Config::send(
    const coconut::smtp::Address& to, const std::vector<Address> cc,
