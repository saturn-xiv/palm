...
coconut::postgresql::Config::Config(const toml::table& node) {
...
std::shared_ptr<coconut::postgresql::Pool> coconut::postgresql::Config::open() {
  std::stringstream ss;
  ...
  auto pool =
      std::make_shared<coconut::postgresql::Pool>(url, this->_pool_size);
  return pool;
}

coconut::postgresql::Pool::Pool(const std::string& url, size_t pool_size) {
