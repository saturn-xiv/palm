...
User={{ user }}
Group={{ user }}
ExecStart=/usr/bin/loquat rpc -n {{ name }} -p {{ port }}
...
void loquat::application::launch_rpc_server(
    const std::string& name, const uint16_t port,
    std::optional<loquat::application::Ssl> ssl, size_t threads) {
  std::shared_ptr<AesHandler> aesHandler = std::make_shared<AesHandler>(name);
  std::shared_ptr<v1::AesProcessor> aesProcessor =
      std::make_shared<v1::AesProcessor>(aesHandler);

  std::shared_ptr<HMacHandler> hmacHandler =
      std::make_shared<HMacHandler>(name);
  std::shared_ptr<v1::HMacProcessor> hmacProcessor =
      std::make_shared<v1::HMacProcessor>(hmacHandler);

  std::shared_ptr<JwtHandler> jwtHandler = std::make_shared<JwtHandler>(name);
