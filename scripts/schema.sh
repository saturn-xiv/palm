...
#     fi
#     mkdir -p $target
#     $PROTOBUF_HOME/bin/protoc -I $WORKSPACE/petunia \
#         -I $PROTOBUF_HOME/include/google/protobuf \
#         --php_out=$target --grpc_out=generate_server:$target \
#         --plugin=protoc-gen-grpc=$PROTOBUF_HOME/bin/grpc_php_plugin \
#         $1
# }
...
function generate_for_morus() {
    echo "generate for morus project"
...
function generate_for_dahlia() {
    echo "generate for morus dahlia project"
    local target=$WORKSPACE/dahlia/src/main/java
    if [ -d $target/com/github/saturn_xiv/palm/plugins ]; then
        rm -r $target/com/github/saturn_xiv/palm/plugins
    fi
    $PROTOBUF_HOME/bin/protoc \
        -I $WORKSPACE/morus -I $WORKSPACE/dahlia \
        -I $PROTOBUF_HOME/include/google/protobuf \
        --java_out=$target --grpc_out=$target \
        --plugin=protoc-gen-grpc=$PROTOBUF_HOME/bin/grpc_java_plugin \
        $WORKSPACE/morus/morus.proto $WORKSPACE/dahlia/dahlia.proto
}
