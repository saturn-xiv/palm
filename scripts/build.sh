...
if [ $ID == "arch" ]; then
    cmake --preset=arch -DVCPKG_TARGET_TRIPLET=x64-linux-release -DVCPKG_CHAINLOAD_TOOLCHAIN_FILE=$WORK_DIR/toolchains/arch/clang.cmake $THRIFT_FLAGS $CASBIN_FLAGS
    cmake --build $WORK_DIR/build/arch
elif [ $ID == "ubuntu" ]; then
    cmake --preset=x86_64 -DVCPKG_TARGET_TRIPLET=x64-linux-release $THRIFT_FLAGS $CASBIN_FLAGS
    ...
    cmake --preset=aarch64 -DVCPKG_TARGET_TRIPLET=arm64-linux-release -DVCPKG_CHAINLOAD_TOOLCHAIN_FILE=$WORK_DIR/toolchains/ubuntu/gcc/aarch64.cmake $THRIFT_FLAGS $CASBIN_FLAGS
