...
#include "palm/models/GroupUser.h"
...
GroupUser::Ptr GroupUser::find(Poco::ActiveRecord::Context::Ptr pContext, const ID& id)
{
	Poco::ActiveRecord::StatementPlaceholderProvider::Ptr pSPP(pContext->statementPlaceholderProvider());
	GroupUser::Ptr pObject(new GroupUser);
...
void GroupUser::insert()
{
	Poco::ActiveRecord::StatementPlaceholderProvider::Ptr pSPP(context()->statementPlaceholderProvider());

	context()->session()
		<< "INSERT INTO group_users (id, version, created_at, updated_at)"
...
const std::vector<std::string>& GroupUser::columns()
{
...
const std::string& GroupUser::table()
{
	static const std::string t = "group_users";
	return t;
}
