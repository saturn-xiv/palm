...
#include "palm/models/RoleRelation.h"
...
RoleRelation::RoleRelation(ID id):
	Poco::ActiveRecord::ActiveRecord<Poco::Int64>(id)
{
}


RoleRelation::RoleRelation(const RoleRelation& other):
...
RoleRelation::Ptr RoleRelation::find(Poco::ActiveRecord::Context::Ptr pContext, const ID& id)
{
	Poco::ActiveRecord::StatementPlaceholderProvider::Ptr pSPP(pContext->statementPlaceholderProvider());
	RoleRelation::Ptr pObject(new RoleRelation);
...
void RoleRelation::insert()
{
	Poco::ActiveRecord::StatementPlaceholderProvider::Ptr pSPP(context()->statementPlaceholderProvider());

	context()->session()
		<< "INSERT INTO roles_relations (id, version, created_at, updated_at)"
...
void RoleRelation::update()
{
	Poco::ActiveRecord::StatementPlaceholderProvider::Ptr pSPP(context()->statementPlaceholderProvider());

	context()->session()
		<< "UPDATE roles_relations"
...
const std::vector<std::string>& RoleRelation::columns()
{
...
const std::string& RoleRelation::table()
{
	static const std::string t = "roles_relations";
	return t;
}
