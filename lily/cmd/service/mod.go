...
func TexToPdfWorkerSystemdConf(name string, queue string) error {
	name = "tex-to-pdf." + name
	tpl, err := template.New("").Parse(`
[Unit]
Description=A tex-to-pdf consumer worker
...
User=root
Group=root
ExecStart=/usr/bin/lily tex-to-pdf-worker -q {{ .queue }} -n {{ .name }}
