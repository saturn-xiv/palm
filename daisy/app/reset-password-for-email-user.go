...
func ResetPasswordForEmailUser(config_file string, email string, password string, debug bool) error {
	...
	if err = db.Transaction(func(tx *gorm.DB) error {
		...
		if err := models.CreateLog(tx, user.UserID, plugin, gl_localhost, v2.UserIndexLogResponse_Item_WARNING, fmt.Sprintf("reset password by %s", cur_usr.Username)); err != nil {
			return err
		}
		return nil
	}); err != nil {
